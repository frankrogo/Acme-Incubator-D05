...
@Service
public class InvestorMessageCreateService implements AbstractCreateService<Investor, Message> {
	...
	public boolean authorise(final Request<Message> request) {
		...
		String nameOwner;
		Principal principal;
		List<String> messengersOfTheForum = new ArrayList<>();
		principal = request.getPrincipal();
		nameOwner = principal.getUsername();

		messengersOfTheForum = (List<String>) this.repository.findUserNamesFromMessengers(request.getModel().getInteger("forumId"));
		result = messengersOfTheForum.contains(nameOwner);
	...
	public void validate(final Request<Message> request, final Message entity, final Errors errors) {
		...
		if (!errors.hasErrors()) {
			...
			errors.state(request, !spamTitle, "title", "investor.message.error.spamTitle");
			errors.state(request, !spamBody, "body", "investor.message.error.spamBody");
			errors.state(request, !spamTags, "tags", "investor.message.error.spamTags");
		}
